...
def train(epo, model, train_loader, optimizer):
    ...
    for it, data in enumerate(train_loader):
        ...
        mask = Variable(mask).cuda(args.gpu)
        nir = Variable(nir).cuda(args.gpu)
        red = Variable(red).cuda(args.gpu)
        ...
        inputs = torch.cat([rgb, ndvi], dim=1)

        optimizer.zero_grad()
        #output,_ = model(rgb)
        output,_ = model(inputs)
        #loss = F.cross_entropy(output, mask)
        mask = torch.sum(mask, dim=1) #torch.Size([3, 3, 240, 240]) -> torch.Size([3, 240, 240])
        mask = mask.long()  #shape: [batch_size, height, width]
        #print(torch.unique(mask)) #tensor([0, 1], device='cuda:0')- 
        loss = F.cross_entropy(output, mask)
        loss.backward()
        optimizer.step()
        ...
        acc = calculate_accuracy(output, mask)
        loss_avg += float(loss)
        acc_avg  += float(acc) 
