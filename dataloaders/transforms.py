...
class ToTensor:
    ...
    def __call__(self,rgb,nir,red,mask):
        ...
        if not torch.is_tensor(rgb):
            nir = cv2.convertScaleAbs(nir)    #can't convert np.ndarray of type numpy.uint16
            red = cv2.convertScaleAbs(red)    #can't convert np.ndarray of type numpy.uint16
            ...
            nir  = nir.type(torch.float32)
            red  = red.type(torch.float32)
...
class Compose:
    ...
    def __call__(self, rgb, nir,red, mask):
        for t in self.transforms:
            rgb,nir, red,mask = t(rgb,nir,red,mask)
        return rgb,nir,red,mask
